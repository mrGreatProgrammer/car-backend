...
logger = logging.getLogger('modelapp.views')


class CategoryList(APIView):
    def get(self, request):
        category = Model.objects.all()
...
class CategoryDetails(APIView):
    def get_object(self, _id):
        return get_object_or_404(Model, id=_id)
    ...
    def put(self, request, _id):
        try:
            ...
            logger.warning(f"Failed to update category. Model with ID {_id} not found.")
            return Response({"message": "Model Not Found"}, status=404)

        serializer = CategorySerializer(category, data=request.data)
        if serializer.is_valid():
            serializer.save()
            logger.info(f"Model with ID {_id} updated successfully.")
            return Response(serializer.data, status=200)
    ...
    def delete(self, request, _id):
        try:
            ...
            logger.warning(f"Failed to delete category. Model with ID {_id} not found.")
            return Response({"message": "Model Not Found"}, status=404)

        category.is_deleted = True
        logger.info(f"Model with ID {_id} marked as deleted.")
        return Response({"message": "Model has been successfully removed."}, status=200)
